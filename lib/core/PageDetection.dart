...
class PageDetection
{
  ...
  void _union(Pair _a, Pair _b, List<List<Pair>> _agent, List<List<int>> _quantity) {
    ...
    _b = _find(_b, _agent);
    if (_a == _b) return;
    if (_quantity[_a[0]][_a[1]] >= _quantity[_b[0]][_b[1]]) {
      _quantity[_a[0]][_a[1]] += _quantity[_b[0]][_b[1]];
      _agent[_b[0]][_b[1]] = _a;
    }
    _quantity[_b[0]][_b[1]] += _quantity[_a[0]][_a[1]];
    _agent[_a[0]][_a[1]] = _a;
  }
  ...
  List <Pair> getPageCoordinates() {
    ...
    while(queue.isNotEmpty) {
      ...
      if (a[0] - distance >= distance && visited[a[0] - distance][a[1]] == false
        ...
        visited[a[0] - distance][a[1]] = true;
      }
      if (a[0] + distance <= ((width - 6) ~/ distance) * distance && visited[a[0] + distance][a[1]] == false
          && _edge(a, Pair(a[0] + distance, a[1]), laplacian, width, height) == false) {
        queue.add(Pair(a[0] + distance, a[1]));
        visited[a[0] + distance][a[1]] = true;
      }
      if (a[1] - distance >= distance && visited[a[0]][a[1] - distance] == false
          && _edge(a, Pair(a[0], a[1] - distance), laplacian, width, height) == false) {
        queue.add(Pair(a[0], a[1] - distance));
        visited[a[0]][a[1] - distance] = true;
      }
      if (a[1] + distance <= ((height - 6) ~/ distance) * distance && visited[a[0]][a[1] + distance] == false
          && _edge(a, Pair(a[0], a[1] + distance), laplacian, width, height) == false) {
        queue.add(Pair(a[0], a[1] + distance));
        visited[a[0]][a[1] + distance] = true;
      }
    ...
    for (int i = distance; i <= ((width - 6) ~/ distance) * distance; i += distance) {
      for (int o = distance; o <= ((height - 6) ~/ distance) * distance; o += distance) {
        if (visited[i][o] == false && quantity[i][o] >= 400) {
          ...
          if (i <= startx && o >= starty &&
              _square(i - startx) + _square(o - starty) >
                  _square(corners[1][0] - startx) +
                      _square(corners[1][1] - starty))
            corners[1] = Pair(i, o);
          if (i <= startx && o <= starty &&
              _square(i - startx) + _square(o - starty) >
                  _square(corners[2][0] - startx) +
                      _square(corners[2][1] - starty))
            corners[2] = Pair(i, o);
          if (i >= startx && o <= starty &&
              _square(i - startx) + _square(o - starty) >
    ...
    if (bigPicture == true) {
      corners[0] = Pair(3 * (corners[0][0] - 10), 3 * (corners[0][1] - 10));
      corners[1] = Pair(3 * (corners[1][0] + 10), 3 * (corners[1][1] - 10));
      corners[2] = Pair(3 * (corners[2][0] + 10), 3 * (corners[2][1] + 10));
      corners[3] = Pair(3 * (corners[3][0] - 10), 3 * (corners[3][1] + 10));
    }
    else {
      corners[0] = Pair(corners[0][0] - 10, corners[0][1] - 10);
      corners[1] = Pair(corners[1][0] + 10, corners[1][1] - 10);
      corners[2] = Pair(corners[2][0] + 10, corners[2][1] + 10);
      corners[3] = Pair(corners[3][0] - 10, corners[3][1] + 10);
    }
